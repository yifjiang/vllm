...
def _apply_moe_activation_masked(
    ...
    _validate_moe_activation_shapes(activation, output, input, expected_dim=3)
    assert expert_num_tokens.dtype == torch.int32, (
        "expert_num_tokens must use torch.int32"
    )
    assert expert_num_tokens.dim() == 1, "expert_num_tokens must be 1D"
    assert expert_num_tokens.size(0) == input.size(0), (
        "expert_num_tokens must have one count per expert"
    )
...
def apply_moe_activation(
    ...
    *,
    ...
    and expert token counts remain per-call inputs because they depend on the
    current token assignment. When ``expert_num_tokens`` is provided, input and
    output use padded ``[E, T, D]`` layouts and only each expert's valid prefix
    is activated.
    """
    config = (
